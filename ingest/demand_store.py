"""JSONL + SQLite soft-demand store."""
...
class DemandStore:
    def __init__(self, root: Path) -> None:
        ...
        self.jsonl_path = self.root / "demand.jsonl"
        self.db_path = self.root / "demand.db"
        self.status_path = self.root / "sources_demand_status.json"
        self._init_db()
    ...
    def matchable(self) -> list[dict[str, Any]]:
        """Soft-demand rows suitable for RFQ board (exclude proxies)."""
        return [
            r
            for r in self.all()
            if r.get("kind") in {"published", "public_rfq", "tender"}
        ]
    ...
    def _rewrite_unlocked(self, rows: list[dict[str, Any]]) -> None:
        ...
        con = sqlite3.connect(self.db_path)
        try:
            con.execute("DELETE FROM demand")
            con.executemany(
                "INSERT INTO demand (id, kind, source, fetched_at, payload) VALUES (?,?,?,?,?)",
                [
                    (
                        r.get("id"),
                        r.get("kind"),
